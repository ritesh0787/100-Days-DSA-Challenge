# 🚀 100 Days DSA Challenge - Placement Preparation

A comprehensive 100-day Data Structures and Algorithms challenge designed to help you prepare for technical interviews and placements. This challenge includes **200+ carefully curated questions** covering all essential topics.

## 📋 Challenge Overview

- **Duration**: 100 Days
- **Total Questions**: 200+ DSA Problems
- **Goal**: Master Data Structures & Algorithms for Placement Preparation
- **Daily Commitment**: 2-3 hours minimum

## 🎯 Challenge Structure

### Phase 1: Foundations (Days 1-30) - 60 Questions
- **Week 1-2**: Arrays & Strings (20 questions)
- **Week 3**: Searching & Sorting (15 questions)
- **Week 4**: Mathematics & Bit Manipulation (15 questions)
- **Week 5**: Recursion & Backtracking (10 questions)

### Phase 2: Intermediate (Days 31-60) - 70 Questions
- **Week 6-7**: Linked Lists (15 questions)
- **Week 8-9**: Stacks & Queues (20 questions)
- **Week 10-11**: Binary Trees (20 questions)
- **Week 12**: Binary Search Trees (15 questions)

### Phase 3: Advanced (Days 61-90) - 60 Questions
- **Week 13-14**: Heaps & Hashing (20 questions)
- **Week 15-16**: Graphs (25 questions)
- **Week 17-18**: Dynamic Programming (15 questions)

### Phase 4: Practice & Mock Interviews (Days 91-100) - 10+ Questions
- **Week 19-20**: Mixed practice & company-specific questions
- Mock interviews and revision

## 📊 Difficulty Distribution

- 🟢 **Easy**: 80 questions (40%)
- 🟡 **Medium**: 90 questions (45%)
- 🔴 **Hard**: 30 questions (15%)

## 📁 Repository Structure

```
100-Days-DSA-Challenge/
├── README.md
├── PROGRESS.md                 # Track your daily progress
├── TOPICS.md                   # Detailed topic-wise question list
├── Resources/                  # Study materials and references
│   ├── Cheatsheets/
│   ├── Articles/
│   └── Videos/
├── Day-001-to-030/            # Phase 1: Foundations
│   ├── Day-001/
│   ├── Day-002/
│   └── ...
├── Day-031-to-060/            # Phase 2: Intermediate
├── Day-061-to-090/            # Phase 3: Advanced
└── Day-091-to-100/            # Phase 4: Practice
```

## 🏆 How to Use This Repository

1. **Fork this repository** to your GitHub account
2. **Clone** your forked repository locally
3. Follow the daily schedule in `TOPICS.md`
4. Solve problems and commit your solutions
5. Update your progress in `PROGRESS.md`
6. Stay consistent and complete the challenge!

## 📝 Daily Routine

1. Read the problem statement carefully
2. Analyze time and space complexity
3. Write clean, well-commented code
4. Test with multiple test cases
5. Commit your solution with a meaningful message
6. Update progress tracker

## 💡 Tips for Success

- ✅ Be consistent - solve problems daily
- ✅ Understand the approach before coding
- ✅ Focus on time and space complexity
- ✅ Review and revise solved problems
- ✅ Practice mock interviews regularly
- ✅ Join study groups for motivation

## 🔗 Recommended Platforms

- [LeetCode](https://leetcode.com/)
- [GeeksforGeeks](https://www.geeksforgeeks.org/)
- [HackerRank](https://www.hackerrank.com/)
- [Codeforces](https://codeforces.com/)
- [InterviewBit](https://www.interviewbit.com/)

## 📈 Track Your Progress

Update your daily progress in `PROGRESS.md`. Mark completed questions with ✅.

## 🎓 Target Companies

This challenge prepares you for interviews at:
- FAANG (Facebook, Amazon, Apple, Netflix, Google)
- Microsoft, Adobe, Uber, Airbnb
- Indian Startups: Flipkart, Swiggy, Zomato, Paytm
- Service Companies: TCS, Infosys, Wipro, Accenture

## 🤝 Contributing

Feel free to:
- Add more questions
- Share your solutions
- Improve documentation
- Report issues

## 📜 License

This project is open source and available for everyone to use for placement preparation.

## 💪 Stay Motivated

> "Success is the sum of small efforts repeated day in and day out." - Robert Collier

**Start Today. Stay Consistent. Get Placed!** 🎯

---
⭐ Star this repository if you find it helpful!
📢 Share with friends preparing for placements!